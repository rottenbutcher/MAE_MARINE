...
at::Tensor ApproxMatchForward(
  ...
  TORCH_CHECK(xyz2.size(0) == b, "batch size of xyz1 and xyz2 must be equal");
  TORCH_CHECK(xyz1.size(2) == 3, "xyz1 must have 3 channels");
  TORCH_CHECK(xyz2.size(2) == 3, "xyz2 must have 3 channels");
  CHECK_INPUT(xyz1);
  CHECK_INPUT(xyz2);
