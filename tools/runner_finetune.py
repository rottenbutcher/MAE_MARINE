...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    if args.use_viewpoint_mask:
        train_transforms = transforms.Compose(
            [
                PointcloudViewpointMasking(viewpoint_mask_ratio=0.5, random_mask_ratio=0.4),
            ]
        )
    ...
    for epoch in range(start_epoch, config.max_epoch + 1):
        ...
        if epoch % args.val_freq == 0 and epoch != 0:
            ...
            # 3. 임계값 돌파 시 동결 해제 및 옵티마이저 재생성
            if encoder_is_frozen and metrics.acc > args.unfreeze_threshold:
                print_log(f'Accuracy {metrics.acc:.4f} > threshold {args.unfreeze_threshold:.4f}. Unfreezing encoder.', logger=logger)
                
                # Encoder 파라미터 활성화
                for param in base_model.module.encoder.parameters():
                    param.requires_grad = True
                
                # 더 낮은 학습률로 옵티마이저 및 스케줄러 재생성
                ...
                encoder_is_frozen = False # 플래그 업데이트
...
def validate(base_model, test_dataloader, epoch, val_writer, args, config, logger = None):
    ...
    if val_writer is not None:
        val_writer.add_scalar('Metric/ACC', acc.item(), epoch)
